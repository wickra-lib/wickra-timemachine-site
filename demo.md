...
description: The Wickra indicator core that Wickra Time Machine is built on, running in your browser — powered by the wickra-wasm bundle from npm.
---

# The core, live

Every line below is computed by **`wickra-wasm`** — the indicator core
Wickra Time Machine is built on — running inside your browser via WebAssembly:
no server, no API call, no install. The same Rust kernel that computes every
indicator Wickra Time Machine reads is producing every value you see.
...
the [API pages here](/api/rust) are Wickra Time Machine's.
